...
export const CategoriesPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await api.categories.update(editId, form);
  ...
  const columns = [
    {
      key: 'code',
      header: 'KODE PREFIX',
      render: (row: any) => <span className="font-mono text-xs text-text-muted">{row.code || '-'}</span>
    },
    {
      key: 'name',
    ...
    },
    {
      key: 'description',
      header: 'DESKRIPSI',
      render: (row: any) => <span className="font-medium text-text-primary text-xs">{row.description || '-'}</span>
    },
    {
      key: 'actions',
      header: 'AKSI',
    ...
    <div className="flex flex-col h-full gap-3 relative">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="page-subtitle mt-0.5">Kelola pengelompokan produk dan barang gudang.</p>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-surface border border-border rounded-sm w-full max-w-md shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="flex flex-col gap-1.5 mb-3">
              ...
              <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!editId) {
                      updates.code = newName.replace(/[^a-zA-Z0-9]/g, '').substring(0, 3).toUpperCase();
                    }
            ...
            <div className="flex flex-col gap-1.5 mb-3">
              <label className="text-xs font-medium text-text-secondary">
                Kode Prefix SKU <span className="text-status-danger">*</span>
              </label>
              <input
                  type="text"
                  value={form.code}
              ...
              <span className="text-[11px] text-text-muted font-sans mt-0.5">
                Kode ini digunakan sebagai awalan otomatis nomor SKU produk (Contoh: ELE-0001).
              </span>
            </div>
