...
export const UomsPage: React.FC = () => {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const res = await api.uoms.getAll();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name || !form.code) {
      addToast({ type: 'error', message: 'Nama Satuan dan Kode/Simbol wajib diisi.' });
      return;
    }
    ...
    try {
      ...
      if (editId) {
        await api.uoms.update(editId, payload);
        addToast({ type: 'success', message: 'Data berhasil diperbarui!' });
      } else {
        await api.uoms.create(payload);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.uoms.delete(deletingItem.id);
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="page-title">Satuan Barang (UoM)</h1>
            <span className="px-2 py-0.5 rounded bg-surface border border-border text-[10px] font-mono text-text-muted uppercase">
              {filteredData.length} DATA
            </span>
          </div>
          <p className="page-subtitle mt-0.5">Kelola unit ukuran (Pieces, Box, Kilogram, Liter).</p>
        </div>
          ...
          <div className="bg-surface border border-border rounded-sm w-full max-w-md shadow-xl flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border bg-surface-subtle">
              <h2 className="font-bold text-text-primary text-sm uppercase">
                {editId ? 'Edit Data Satuan' : 'Tambah Satuan Baru'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col p-5">
              ...
              <div className="flex flex-col gap-1.5 mb-3">
                <label className="text-xs font-medium text-text-secondary">
                  Nama Satuan <span className="text-status-danger">*</span>
                </label>
                <input
                  type="text"
                  ...
                  className="h-9 px-3 w-full bg-surface border border-border rounded-sm text-xs font-mono focus:outline-none focus:ring-1 focus:ring-accent uppercase"
                />
              </div>

              <div className="flex flex-col gap-1.5 mb-3">
                <label className="text-xs font-medium text-text-secondary">
                  Kode / Simbol Satuan <span className="text-status-danger">*</span>
                </label>
                <input
                  type="text"
